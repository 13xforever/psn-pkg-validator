...
{
    public static readonly byte[] NpdrmPubKey = (
        "E6792E446CEBA27BCADF374B99504FD8E80ADFEB" + "3E66DE73FFE58D3291221C65018C038D3822C3C9"
    ).AsBytes();
    public static readonly byte[] NpdrmPubKeyOld = (
        "D9AAEB6054307FC0FB488B15AE11B558C75FC8A3" + "EC4907E129C5B5CD386D94D82318B9D558777C5A"
    ).AsBytes();
    public static readonly byte[] VshPubKey = (
        "6227B00A02856FB04108876719E0A0183291EEB9" + "6E736ABF81F70EE9161B0DDEB026761AFF7BC85B"
    ).AsBytes();
    public static ReadOnlyECPointRef NpdrmQ => CreateReadOnlyPointRef(NpdrmPubKey);
    ...
    private static readonly byte[] VshCurve1Data = (
        "0000000000000000FFFFFFFE0000000000000000" +
        ...
        "9FBCA7BA92F5E34D6F7216F0D828A37D413EF73F"
    ).AsBytes();

    private static readonly byte[] VshCurve2Data = (
        "0000000000000000FFFFFFFE0000000000000000" +
        "0000000000000000FFFFFFFE0000000000000003" +
        "5974123CCBE7FD63E2C31CC465CDE0334461F0F4" +
        "000000000000000100004A51C3ADC19C6BB0DED8" +
        "ED713BDA9B780270209B1DBC843F5E092A5021D3" +
        "A6A7AA814E24FFED9FBDAADB243C862A53A0B520"
    ).AsBytes();

    private static readonly byte[] VshInvCurve1Data = (
    ...
    private static readonly byte[] VshInvCurve2Data = (
        "FFFFFFFFFFFFFFFF00000001FFFFFFFFFFFFFFFF" + // p
        ...
        "5958557EB1DB001260425524DBC379D5AC5F4ADF"   // gy
    ).AsBytes();
    ...
    private static Ecdsa CreateCurve(Span<byte> curveData)
        => new(
            ...
            curveData[0x28..0x3c],
            curveData[0x3c..0x50],
            CreatePoint(curveData[0x50..0x78])
        );

    public static ECPoint CreatePoint(ReadOnlySpan<byte> pointData)
        => new()
        {
            X = pointData[0x00..0x14].ToArray(),
            Y = pointData[0x14..0x28].ToArray(),
        };

    public static ReadOnlyECPointRef CreateReadOnlyPointRef(ReadOnlySpan<byte> pointData)
        => new(pointData[0x00..0x14], pointData[0x14..0x28]);
    ...
    public static byte[] AsBytes(this string hexString)
    {
        ...
        if (hexString.Length is 0)
            return [];

        if (hexString.Length%2 is 1)
